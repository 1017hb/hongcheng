  ...
  */
  getConfirm:function(){
    ...
    orderConfirm(this.data.cartId).then(res=>{
      console.log('结算页面0000000000000000000')
      console.log(res)

      let data = res.data;
      that.setData({
        userInfo: data.userInfo,
        integral: data.userInfo.integral,
        cartInfo: data.cartInfo,
        integralRatio: data.integralRatio,
        offlinePostage: data.offlinePostage,
        orderKey: data.orderKey,
        priceGroup: data.priceGroup,
        totalPrice: app.help().Add(parseFloat(data.priceGroup.totalPrice), parseFloat(data.priceGroup.storePostage)),
        seckillId: parseInt(data.seckill_id),
        usableCoupon: data.usableCoupon,
        system_store: data.system_store,
        store_self_mention: data.store_self_mention
      });
      that.data.cartArr[1].title = '可用余额:' + data.userInfo.now_money;
      
      if (data.offline_pay_status == 2)  that.data.cartArr.pop();
      that.setData({ cartArr: that.data.cartArr, ChangePrice: that.data.totalPrice });
      that.getBargainId();
      that.getCouponList();
      if (Object.keys(data.cartInfo).length > 0){
        that.setData({
          isShowOper:true
        })
        wx.hideLoading();
    ...
    setTimeout(function(){
      if (that.data.cartInfo.length > 0) {
        for (let i = 0; i < that.data.cartInfo.length; i++) {
          if (that.data.cartInfo[i].product_id === 301 || that.data.cartInfo[i].product_id === 302 || that.data.cartInfo[i].product_id === 656) {
            vip_truePrice += that.data.cartInfo[i].vip_truePrice;
          }
        }
      }
      let money = parseFloat(that.data.priceGroup.vipPrice);
      that.setData({
        vip_truePrice: (money - vip_truePrice).toFixed(2)
      })
    },2000)

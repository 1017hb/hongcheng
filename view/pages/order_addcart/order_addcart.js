  ...
  */
  ...
  switchSelect:function(){
    ...
    if (selectValue.length < 1) {
       that.setData({ selectCountPrice: selectCountPrice });
    }else{
      console.log('12121212121')
      console.log(validList)
      for (var index in validList){
        if (that.inArray(validList[index].id, selectValue)){
          if (validList[index].productInfo.id === 301 || validList[index].productInfo.id === 302){
            selectCountPrice = Number(selectCountPrice) + Number(validList[index].productInfo.price)
          }else {
  ...
  subCart:function(event){
    ...
    console.log(item);
    item.cart_num = item.cart_num - item.batch;
    if (item.cart_num < item.batch) status = true;
    if (item.cart_num <= item.batch) { 
      item.cart_num = item.batch;
      item.numSub = true; 
  ...
  addCart: function (event) {
    ...
    if (productInfo.hasOwnProperty('attrInfo') && item.cart_num >= item.productInfo.attrInfo.stock) {
      ...
      item.numAdd = true;
      item.numSub = false; 
    } else if (item.product_id === 301 || item.product_id === 302 || item.product_id === 656){
      item.cart_num = 1;
    }else { 

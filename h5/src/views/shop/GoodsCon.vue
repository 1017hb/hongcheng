    ...
    <div class="wrapper">
      <div class="share acea-row row-between row-bottom">
        <div class="money font-color-red">
			<div class="vip-name">{{vip_name}}会员</div> 
			<div class="num">￥{{vip ? storeInfo.vip_price : storeInfo.price}}</div>
			<div v-if="!vip" style="padding-top: 0.18rem;">
				<span
					class="vip-money"
		...
		<div class="ico-share" @click="listenerActionSheet">
      ...
      <div class="label acea-row row-between-wrapper">
        <div>吊牌价:￥{{ storeInfo.ot_price }}</div>
        <div>库存:{{ storeInfo.stock }}{{ storeInfo.unit_name }}</div>
        <div>销量:{{ storeInfo.fsales }}{{ storeInfo.unit_name }}</div>
      </div>
	  <div class="coupon on" v-if="vip_list.length">
		  <div class="acea-row row-between-wrapper vip-title">
			  <div class="form">会员专享</div>
			  <div class="form on">
				<div style="padding: 0.10rem 0rem;">享受会员折扣价</div>
				<div class="open-vip">开通会员</div>  
			  </div>
		  </div>
		  <div class="acea-row row-between-wrapper">
			  <div v-for="(item , index) in vip_list" class="vip-item " :class="index==0? 'active':'' ">
				  <div class="vip-item-top">{{item.title}}</div>
					  <div class="vip-item-bottom">
						  <div class="vip-item-title">{{item.intro}}</div>
						  <div class="vip-item-price">￥{{item.price}}</div>  
					  </div>
			  </div>
		  </div>
		  <div class="intro row-between-wrapper">
		  		<div class="intro-text">说明</div>
					<div class="intro-img">
						<img src="@assets/images/intro.png" />
						<div class="intro-info">购买对应会员后所有的商品都可以享受对应的折扣优惠(促销商品除外)，另随机赠送一件商品</div>
					</div>
				</div>
		  </div>
...
.product-con .wrapper .share .ico-share{display: flex}
.product-con .wrapper .share .ico-text{line-height: 0.46rem;width: 0.7rem;color: red}
.product-con .wrapper .share .ico-share .icon-fenxiang{color: red}
...
.product-con .wrapper .coupon.on{
  margin-top: 0rem;
  height: 6.25rem;
}
.product-con .wrapper .coupon .form{
  padding: 0.25rem 0 0.2rem 0;
  font-size: 0.30rem;
}
.product-con .wrapper .coupon .form.on{
  font-size: 0.25rem;
  display: flex;
}
.vip-item{
...
.vip-item .vip-item-top{
  background-color: #c7c5c5;
  color: #fff;
  border: 0.01rem solid #ebeaea;
  line-height: 1.00rem;
}
.acea-row .vip-item:nth-child(1) .vip-item-top{
  border-right: 0;
}
.acea-row .vip-item:nth-child(2) .vip-item-top{
  border-right: 0;
  border-left: 0;
}
.vip-item.active .vip-item-top{
  background-color: red;
  color: #fff;
  border: 0.01rem solid red;
}
.acea-row:nth-child(1) .vip-item-top{
  border-left: 0;
}

.vip-item-bottom{
  border: 0.01rem solid #ddd9d9;
  border-top: 0;
}
.vip-item-bottom .vip-item-title{
  border-bottom: 0.01rem solid #ddd9d9;
  line-height: 1.20rem;
}
.vip-item .vip-item-bottom .vip-item-price{
  line-height: 1.20rem;
}

.acea-row .vip-item:nth-child(1) .vip-item-bottom{
  border-right: 0;
}
.acea-row .vip-item:nth-child(2) .vip-item-bottom{
  border-right: 0;
}
.vip-item.active .vip-item-bottom{
  border: 0.01rem solid red;
  border-top: 0;
  color: red;
}
.vip-item.active .vip-item-bottom .vip-item-title{
  border-bottom: 0.01rem solid red;
}
.acea-row .vip-item.active:nth-child(1) .vip-item-bottom{
  border-right: 0.01rem solid red;
}
.acea-row .vip-item.active:nth-child(2) .vip-item-bottom{
  border-right: 0.01rem solid red;
}
.acea-row .vip-item.active:nth-child(3) .vip-item-bottom{
  border-left: 0;
}
.vip-title{
  color: black;
...
.intro-text{
  padding:0.30rem 0 0.03rem 0;
}
.intro-img img{
  width: 0.40rem;
  height: 0.40rem;
  padding: 0.4rem 0.1 0 0.12rem;
}
.intro-info{
  margin-left: 0.10rem;
  line-height: 0.40rem;
      ...
      getProductDetail(that.id)
        .then(res => {
		  ...
		  let vip_list = [
			...
			vip_list[0].price = storeInfo.price;
			vip_list[1].price = (storeInfo.price * 0.98).toFixed(2)
			vip_list[2].price = (storeInfo.price * 0.95).toFixed(2)
			that.vip_list = vip_list;
